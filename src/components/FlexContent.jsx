...
const FlexContent = ({ ifExist, endpoint: { title, heading, subtitle, text, img, btn, url }}) => {
  return (
    <>
        ...
        `}>
            <div className='max-w-lg lg:min-w-none w-full md:text-center grid items-center lg:justify-items-center'>
                <h1 className='text-4xl sm:text-3xl font-bold text-gradient'>{heading}</h1>
                <h1 className='text-5xl lg:text-4xl md:text-3xl sm:text-2xl font-bold text-slate-900 filter'>{title}</h1>
            ...
            <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
                <img 
                    ...
                    className={`w-auto object-fill transitions-theme 
                        ${ifExist ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}
                    `}
                />
            </div>
        </div>
